...
grps_mod <- MRFcov(data = grps_mat, n_nodes = length(groupers), n_covariates = 4, family = "poisson")
...
herb_mod <- MRFcov(data = herb_mat, n_nodes = length(herbivores), n_covariates = 4, family = "poisson")
...
# Species co-occurrence with and without covariates -----------------------

# GROUPERS
grpsHM_cov <- plotMRF_hm(grps_mod, main = "with covariates")
grps_mod_nocov <- MRFcov(data = grps_mat[1:length(groupers)], n_nodes = length(groupers), family = "poisson")
...
dip_mod_nocov <- MRFcov(data = dip_mat[1:length(diplodus)], n_nodes = length(diplodus), family = "poisson")
...
herb_mod_nocov <- MRFcov(data = herb_mat[1:length(herbivores)], n_nodes = length(herbivores), family = "poisson")
