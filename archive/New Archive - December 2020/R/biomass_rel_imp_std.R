...
grps_bioms_mod <- MRFcov(grps_biomat, prep_covariates = TRUE, n_nodes = 5,
                                 n_covariates = 4, family = "gaussian")

lapply(grps_bioms_mod$key_coefs, function(x) x %>% 
         filter(Rel_importance > 0.1) %>% 
         filter(str_detect(string = Variable, pattern = "_")))

dip_biomat <- create_biomass_mat(dataset = med_biomass,
                                  taxa = diplodus,
                                  covariate = c("mpa", "temp", "depth", "prod"))

dip_mod_std <- MRFcov(dip_biomat, n_nodes = 5, family = "gaussian")
...
herb_mod_std <- MRFcov(herb_biomat, n_nodes = 5, family = "gaussian")
...
grps_relimp <- rel_imp_sum(grps_bioms_mod)
dip_relimp <- rel_imp_sum(dip_mod_std)
herb_relimp <- rel_imp_sum(herb_mod_std)
...
# ggsave("biomass_relimp_nonspat_std.png", species_relimp_plots, "png", "figures/rel_imp/", dpi = 150, height = 10, width = 10, units = "in")
