...
```{r}
spp_mat <- create_spp_mat(dataset = med_raw, basin = east,
                          group = diplodus, covariate = "depth") %>% na.omit()
...
categories <- categorise_cov(species_mat = spp_mat, covariate = "depth") # FIXME Error in cut(): breaks are not unique https://stackoverflow.com/questions/16184947/cut-error-breaks-are-not-unique
nested_cats <- nested_data(categorised_data = categories)
...
```{r}
spp_mat <- create_spp_mat(dataset = med_raw, basin = east,
                          group = herbivores, covariate = "depth") %>% na.omit()
...
png("figures/herb_west_temp.png", height = 450, width = 1200, res = 150)
plot_multi_graphs(nested_df = nested_plot, n_graphs = 3)
dev.off()
...
```{r}
spp_mat <- create_spp_mat(dataset = med_raw, basin = west,
                          group = herb_west, covariate = "depth") %>% na.omit()
...
categories <- categorise_cov(species_mat = spp_mat, covariate = "depth")
# FIXME Error in cut(): breaks are not unique https://stackoverflow.com/questions/16184947/cut-error-breaks-are-not-unique
nested_cats <- nested_data(categorised_data = categories)
