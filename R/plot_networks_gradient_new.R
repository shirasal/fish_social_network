...
plotMRF_net_cont = function(data, MRF_mod, node_names, covariate, main, cutoff, plot){
  ...
  cont.cov.mats <- lapply(observed_cov_quantiles, function(j){
    ...
    net.plot <- plot_graph_new(matrix = pred_values)
  })
  ...
  print(ggpubr::ggarrange(plotlist = cont.cov.mats, ncol = length(observed_cov_quantiles), nrow = 1))
}

plotMRF_net_cont(grps_mat, grps_pois, node_names = groupers, covariate = "temp", plot = TRUE)
# ggsave("figures/networks/groupers_network_temp.png", dpi = 300, scale = c(2,1))
# ggsave("figures/networks/groupers_network_temp.pdf", dpi = 300, scale = c(2,1))
...
# ggsave("figures/networks/diplodus_network_temp.pdf", dpi = 300, scale = c(2,1))
