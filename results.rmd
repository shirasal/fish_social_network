...
## Association evaluation from MRF model  
(Clark _et al._ [^1].)  
...
plotMRF_hm(model_temp_grps$mod, main = "Groupers - association coefficients")
plotMRF_hm(model_temp_grps$boot, main = "Groupers")
...
plotMRF_hm(model_temp_dip$mod, main = "Seabreams - association coefficients")
plotMRF_hm(model_temp_dip$boot, main = "Seabreams")

# Herbivores --------------------------------------------------------------

model_temp_herb <- run_mod(species_mat = herb_temp_mat, n_covs = 1,
                           family = "gaussian", coords = herb_temp_coords)
plotMRF_hm(model_temp_herb$mod, main = "Herbivores - association coefficients")
plotMRF_hm(model_temp_herb$boot, main = "Herbivores")

```
<!--*Groupers*-->
<!--*Seabreams*-->
*Herboivores*: 
Only two _Siganus rivulatus_ is associated with other species ( _Scarus ghobban_ and _Sparisoma cretense_), other herbivore species
...
temp_connect <- tibble(grps_connect = unlist(grps_temp_nested_mod$connectance)) %>% 
  add_column(network = factor(c("low", "medium", "high"))) %>% 
  add_column(dip_connect = unlist(dip_temp_nested_mod$connectance)) %>% 
  add_column(herb_connect = unlist(herb_temp_nested_mod$connectance)) %>% 
  select(network, grps_connect, dip_connect, herb_connect)

formattable(temp_connect)
...
```{r}
mpa_connect <- tibble(grps_connect = unlist(grps_mpa_nested_mod$connectance)) %>% 
  add_column(network = factor(c("out", "in"))) %>% 
  add_column(dip_connect = unlist(dip_mpa_nested_mod$connectance)) %>% 
  add_column(herb_connect = unlist(herb_mpa_nested_mod$connectance)) %>% 
  select(network, grps_connect, dip_connect, herb_connect)
formattable(mpa_connect)
...
formattable(groupers_spp_relimp)
...
formattable(groupers_spp_meancoef)
...
herb_spp_meancoef %<>%
  rename(temp = sum_mean_coefs_temp, mpa = sum_mean_coefs_mpa, depth = sum_mean_coefs_depth)

formattable(herb_spp_meancoef)

```

_Siganus rivulatus_ coefficients are very high compared to other species. This is also the only species that seem to have a species asoociation coefficient that is different from 0 (see [heatmap](#Association evaluation from MRF model)
