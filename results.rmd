...
In order to examine the association between species and how this association changes along environmental gradient, I chose three groups of representative fish species (taxa): groupers (Epinephelus species + Serranus species), seabream species and herbivore species.  
...
test <- (grps_temp_dep[,1:5] != 0)*1
test %>% colSums() %>% 
  knitr::kable(digits = 0, caption = "Count of observations per species", col.names = "obs", format = "pandoc")
rm(test)
# Same for temp and mpa

```

## Compare association coefficients

Association coefficients describe the relationship between species. First it is calculated for the whole dataset and the mean coefficient is extracted:

```{r, message='hide', warning='hide'}

# Temperature and depth ---------------------------------------------------

grps_temp_dep_model <- MRFcov(data = grps_temp_dep, n_nodes = 5, n_covariates = 2, family = "gaussian")
grps_temp_dep_cooccur <- plotMRF_hm(grps_temp_dep_model, main = "Temperature (incl. depth)")

# MPAs and depth ----------------------------------------------------------

grps_mpa_dep_model <- MRFcov(data = grps_mpa_dep, n_nodes = 5, n_covariates = 2, family = "gaussian")
grps_mpa_dep_cooccur <- plotMRF_hm(grps_mpa_dep_model, main = "MPAs (incl. depth)")

gridExtra::grid.arrange(grps_temp_dep_cooccur, grps_mpa_dep_cooccur, nrow = 1, top = "Groupers association coefficients")

```

And also while accounting for spatial autocorrelation
```{r, message='hide', warning='hide'}

# Temperature and depth ---------------------------------------------------

grps_temp_dep_model_spt <- MRFcov_spatial(data = grps_temp_dep, n_nodes = 5, n_covariates = 2, 
                                          family = "gaussian", coords = coords_all)
grps_temp_dep_cooccur_spt <- plotMRF_hm(grps_temp_dep_model_spt, main = "Temperature (incl. depth)")

# MPAs and depth ----------------------------------------------------------

grps_mpa_dep_model_spt <- MRFcov_spatial(data = grps_mpa_dep, n_nodes = 5, n_covariates = 2, 
                                         family = "gaussian", coords = coords_all)
grps_mpa_dep_cooccur_spt <- plotMRF_hm(grps_mpa_dep_model_spt, main = "MPAs (incl. depth)")

gridExtra::grid.arrange(grps_temp_dep_cooccur_spt, grps_mpa_dep_cooccur_spt, nrow = 1, top = "Groupers association coefficients (with spatial)")

```

There are a lot less associations when we account for spatial autocorrelation. Also the association coefficient of the species in the MPA model is relatively lower when accounting for spatial autocorrelation. The relationship between *Epinephelus costae* and *Epinephelus marginatus* seem to be strongly negative in all cases.  

Now the same association coefficients are calculated for each of the covariates levels

```{r}
# Temperature -------------------------------------------------------------
grps_temp_cats <- categorise_cov(species = grps_temp_dep, covariate = "tmean_reg")
grps_temp_cat_nested <- nested_data(categorised_data = grps_temp_cats, ncov = 1)
grps_temp_cat_model <- get_model(data = grps_temp_dep, ncov = 2, coords = coords_all)
grps_temp_nested_mod <- nested_models(nested_df = grps_temp_cat_nested, ncov = 2, coords = coords_all)
...
# MPAs --------------------------------------------------------------------
grps_mpa_cat_nested <- nested_data(categorised_data = as.data.frame(grps_mpa_dep), ncov = 2)
grps_mpa_cat_model <- get_model(data = grps_mpa_dep, ncov = 2)
grps_mpa_nested_mod <- nested_models(nested_df = grps_mpa_cat_nested, ncov = 2)
...
Now take a look at the results for temperature:
```{r}
low_temp_grps_coefs <- sapply(X = grps_temp_nested_mod$model[[1]]$key_coefs, FUN = mean_coef)
med_temp_grps_coefs <- sapply(X = grps_temp_nested_mod$model[[2]]$key_coefs, FUN = mean_coef)
high_temp_grps_coefs <- sapply(X = grps_temp_nested_mod$model[[3]]$key_coefs, FUN = mean_coef)

temp_df <- data.frame(low_temp = low_temp_grps_coefs[1:5],
                      medium_temp = med_temp_grps_coefs[1:5],
                      high_temp = high_temp_grps_coefs[1:5])


formattable(temp_df, align = c("l","c", "c", "c"), list(
  `low_temp` = col_formatter,
  `medium_temp` = col_formatter,
  `high_temp` = col_formatter,
...
And the results for MPAs:
```{r}
notmpa_grps_coefs <- sapply(X = grps_mpa_nested_mod$model[[1]]$key_coefs, FUN = mean_coef)
ismpa_grps_coefs <- sapply(X = grps_mpa_nested_mod$model[[2]]$key_coefs, FUN = mean_coef)

mpa_df <- data.frame(not_mpa = notmpa_grps_coefs[1:5],
                      is_mpa = ismpa_grps_coefs[1:5])


formattable(mpa_df, align = c("l","c", "c"), list(
  `not_mpa` = col_formatter,
  `is_mpa` = col_formatter
))

```

The mean associations of *Epinephelus marginatus* and *Serranus cabrilla* are positive inside and outside marine protected areas. For *Mycteroperca rubra* the mean association is negative both in and out of reserves and there is a slight change in the value compared to all other species.
